...
function toLogPayload(value) {
  if (value === undefined || value === null) return "";
  const text = typeof value === "string" ? value : JSON.stringify(value);
  return String(text).slice(0, 1000);
}

function normalizedCloudflareZoneName(value) {
  return valueText(value).trim().replace(/\.+$/, "").toLowerCase();
}

function cloudflareZoneName(zone) {
  return normalizedCloudflareZoneName(zone?.name || zone?.display || zone?.domain || zone?.zone || zone);
}

function cloudflareRecordNameForZone(dnsHostName, zoneName) {
  return dnsHostName.slice(0, -(zoneName.length + 1));
}
...
function createOperationHelpers({ logLine, operationPollMs, operationTimeoutSeconds }) {
  ...
  async function requestContainerOperation(client, container, operation, prefix) {
    ...
    if (operationTimeoutSeconds > 0) await delay(operationPollMs);
  ...
  function registryId(registry) {
    return registry?.id || registry?.pk || registry?.value || "";
  }

  function registryHostFromImage(image) {
    const ref = String(image || "").trim();
    const firstPart = ref.split("/")[0] || "";
    const hasRegistryHost = firstPart.includes(".") || firstPart.includes(":") || firstPart === "localhost";
    const host = hasRegistryHost ? firstPart.toLowerCase() : "docker.io";
  ...
  function registryText(registry) {
    return [
      ...
      registry?.host,
      registry?.hostname,
    ].map(valueText).join(" ").trim().toLowerCase();
  }

  function registryHostId(registry) {
    const host = registry?.host || "";
    return host?.id || host || "";
  }

  function registryMatchesHost(registry, host) {
    const text = registryText(registry);
    if (text.includes(host)) return true;
    if (host !== "docker.io") return false;
    return dockerRegistryAliases.some((alias) => text.includes(alias));
  }

  async function registryForImageReference(client, image, hostId = "") {
    const host = registryHostFromImage(image);
    try {
      const registries = await client.list("/api/plugins/docker/registries/", { limit: 1000 });
      const matchingRegistries = registries.filter((registry) => registryMatchesHost(registry, host));
      const match = matchingRegistries.find((registry) => String(registryHostId(registry)) === String(hostId)) || matchingRegistries[0];
      return registryId(match);
    } catch {
      return "";
    }
  }
  ...
  async function registryForImage(client, target, image) {
    ...
    const hostMatches = hostId ? await client.list("/api/plugins/docker/images/", { name: image, host_id: hostId, limit: 1 }, { paginate: false }) : [];
    const hostRegistry = imageRegistryId(hostMatches[0]);
    if (hostRegistry) return hostRegistry;

    const globalMatches = await client.list("/api/plugins/docker/images/", { name: image, limit: 1 }, { paginate: false });
    return imageRegistryId(globalMatches[0]) || registryForImageReference(client, image, hostId);
  }
  ...
  async function createDnsRecord(client, data, host) {
    ...
    try {
      ...
      const expectedZoneName = normalizedCloudflareZoneName(zoneName);
      const matchingZones = zones.filter((zone) => {
        const returnedZoneName = cloudflareZoneName(zone);
        return !returnedZoneName || returnedZoneName === expectedZoneName;
      });
      let zone = matchingZones.find((item) => cloudflareZoneName(item) === expectedZoneName);
      if (!zone && matchingZones.length === 1) [zone] = matchingZones;
      /* c8 ignore next */
      if (zone) {
        const hostname = valueText(host.display || host.name || host);
        const recordName = cloudflareRecordNameForZone(dnsHostName, zoneName);
      ...
      const returned = zones.map((item) => cloudflareZoneName(item)).filter(Boolean).join(",") || "none";
      logLine(`CREATE : Cloudflare zone not found for ${zoneName} while creating ${dnsName} count=${zones.length} returned=${returned}`);
    } catch (error) {
      const requestPayload = toLogPayload(error?.request);
      const responsePayload = toLogPayload(error?.payload);
      const requestInfo = requestPayload ? `request=${requestPayload} ` : "";
      const payloadInfo = responsePayload ? `payload=${responsePayload} ` : "";
      logLine(`CREATE : Cloudflare DNS record failed for ${dnsName} ${error.message || "unknown error"} ${requestInfo}${payloadInfo}`);
    }
  }
