...
function cloudflareRecordNameForZone(dnsHostName, zoneName) {
  return dnsHostName.toLowerCase().endsWith(`.${zoneName.toLowerCase()}`)
    ? dnsHostName.slice(0, -(zoneName.length + 1))
    : dnsHostName;
}
...
function createOperationHelpers({ logLine, operationPollMs, operationTimeoutSeconds }) {
  ...
  function registryMatchesHost(registry, host) {
    ...
    return host === "docker.io" && (
      ...
      || text.includes("dockerhub")
      || text.includes("hub.docker.com")
      || text.includes("registry.hub.docker.com")
      || text.includes("index.docker.io")
  ...
  async function createDnsRecord(client, data, host) {
    ...
    try {
      ...
      const zone = matchingZones.find((item) => cloudflareZoneName(item) === expectedZoneName) || (matchingZones.length === 1 ? matchingZones[0] : null);
      if (!zone) {
