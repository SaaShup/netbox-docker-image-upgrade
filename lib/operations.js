...
function createOperationHelpers({ logLine, operationPollMs, operationTimeoutSeconds }) {
  ...
  async function waitForImagePulled(client, image, displayName) {
    ...
    while (Date.now() < deadline) {
      ...
      if (pulledIdentifier) {
        ...
        logLine(`RECREATE : image ${displayName} pulled identifier=${identifierText}`);
        return payload;
      }
      await delay(operationPollMs);
    }
    logLine(`RECREATE : image ${displayName} pull timeout after ${operationTimeoutSeconds}s`);
  ...
  async function ensureImageOnHost(client, oldImage, image, version) {
    ...
    const displayName = `${image}:${version} on ${hostName(oldImage)}`;
    logLine(`RECREATE : created image ${displayName} status=${statusCode}`);
    return waitForImagePulled(client, created, displayName);
  }
