...
class NetBoxClient {
  ...
  async request(method, apiPath, { query = {}, body, expected = [200, 201, 202, 204] } = {}) {
    ...
    if (!expected.includes(response.status)) {
      ...
      const error = new Error(`NetBox request failed ${response.status} ${method} ${requestPath}`);
      error.statusCode = response.status;
      error.payload = payload;
      error.request = { method, path: requestPath, status: response.status };
      throw error;
    }
    return { statusCode: response.status, payload };
  }

  async list(apiPath, query = {}, options = {}) {
    const items = [];
