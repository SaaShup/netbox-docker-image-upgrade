...
function compactStateForWrite(state) {
  ...
  ["templates", "workflows", "order_counts", "enrollment_counts", "order_instances", "enrollment_instances"].forEach((key) => {
...
function createStateStore(dataPath) {
  ...
  function migrateLegacyState() {
    ...
    return {
      ...
      enrollment_counts: plainObject(legacy.enrollment_counts),
      order_instances: plainObject(legacy.order_instances),
      enrollment_instances: plainObject(legacy.enrollment_instances),
