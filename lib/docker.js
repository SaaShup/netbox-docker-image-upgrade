...
function templateNameFromForm(data) {
  ...
  if (!image) return String(data.instance || "").trim();
  const slashIndex = image.lastIndexOf("/");
  const colonIndex = image.lastIndexOf(":");
  let imageName = image;
  if (colonIndex > slashIndex) imageName = image.slice(0, colonIndex);
...
function isCanonicalTemplateLabel(key) {
  const normalized = String(key).trim().toLowerCase();
  if (normalized.startsWith("saashup.template.")) return true;
  return normalized.startsWith("saashup_template_");
}
