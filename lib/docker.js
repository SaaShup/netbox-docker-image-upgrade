...
function containerConfigPayloadFromForm(data, containerId) {
  ...
  const baseLabels = [
    ...
    { key: `traefik.http.middlewares.${name}.ipallowlist.sourcerange`, value: cloudflareSourceRanges },
  ...
  const env = envKeys
    ...
    .filter((item) => item.var_name && item.var_name !== "SAASHUP_OWNER");

  if (data.saashup_owner) {
    env.push({ var_name: "SAASHUP_OWNER", value: String(data.saashup_owner) });
  }
